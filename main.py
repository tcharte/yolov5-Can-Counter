...
def main():
    frame_processor = TrainedModelFrameProcessor(*network_params)
    ...
    detections_queue = multiprocessing.Queue(maxsize=100000)
    count_queue = multiprocessing.Queue(maxsize=1)

    print('Initializing counting process...')
    count_process = multiprocessing.Process(target=count_cans, args=(splits, detections_queue, count_queue))
    count_process.start()

    print('Initializing detection process...')
    detection_process = multiprocessing.Process(target=detect_cans, args=(frame_queue, detections_queue, network_params))
    ...
    start_id = 0
    while True:
        total_cans, current_frame_id = count_queue.get()
        t2 = time.time_ns()/1e9
        elapsed_time = t2-t1
        total_time = t2-start

        if elapsed_time >= refresh_time:
            dropped_frames = frame_queue.qsize()
            ...
            print('Backed-up frames:', dropped_frames)
            print('Backed-up detections:', detections_queue.qsize())
